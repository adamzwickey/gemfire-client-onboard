...
create region --name=PaymentTypeConfig --type=REPLICATE --cache-loader=com.tmo.payments.server.cacheevents.PaymentTypeConfigCacheLoader --entry-idle-time-expiration=129600 --entry-idle-time-expiration-action=destroy --enable-statistics=true

# Payments will survive node outages and has an event queue attached
create region --name=Payments --type=PARTITION_REDUNDANT --async-event-queue-id=1

create region --name=PaymentDuplicateKeys --type=PARTITION_REDUNDANT
